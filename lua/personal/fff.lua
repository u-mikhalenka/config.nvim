...
cfg.pack_add({
    ...
    keys = {
        { "<leader>Ff", function() require('fff').find_files({ resume = true }) end, desc = 'FFFind files' },
        { "<leader>Fg", function() require('fff').live_grep({ resume = true }) end,  desc = 'LiFFFe grep' },
        {
            "<leader>Fz",
            function() require('fff').live_grep({ resume = true, grep = { modes = { 'fuzzy', 'plain' } } }) end,
